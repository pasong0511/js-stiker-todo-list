  ...
  createSticker() {
    ...
    stickerBoardEl.addEventListener("click", (event) => {
      ...
      if (event.target.className === "listCreateBtn") {
        ...
        const ItemListEl = new ItemList(this.getStickerListNum(), stickerBottomEl, this);
        return
      }
...
class ItemList {
  constructor(listNum, stickerBottomEl, parentStiker) {
    this.parentStiker = parentStiker.trelloNum; //트렐로 번호
  ...
  createViewItem() {
    ...
    itemTitleEl.addEventListener("click",
      (event) => {
