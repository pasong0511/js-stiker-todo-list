...
class Sticker {
  ...
  createSticker() {
    ...
    listCreateBtn.addEventListener("click", () => {
      const ItemListEl = new ItemList(stickerBottomEl, this.getStickerListNum(), stickerBoardEl);
    });
...
class ItemList {
  constructor(stickerBoardEl, listNum, parentEl) {
    this.tatgetstickerBoardEl = stickerBoardEl;
    this.listNum = listNum;
    this.createViewItem();
    this.parentSticker = parentEl;
  }
  ...
  createViewItem() {
    ...
    const itemText = createEl("span", "itemText");
    ...
    itemlist.addEventListener("mousedown", (event) => {
      ...
      const clinetReact = itemlist.getBoundingClientRect();
      const shiftX = event.clientX - clinetReact.x;
      const shiftY = event.clientY - clinetReact.y;
      itemlist.style.position = 'fixed';      //이후 position static으로 변경

      const orgZIndex = itemlist.style.zIndex;
      itemlist.style.zIndex = 1000;


      //placehorderEl 생성
      const placehorderEl = document.createElement("div");
      placehorderEl.classList.add("placehorder");

      placehorderEl.style.left = itemlist.style.left; //placehorder 위치 찾기 -> itemlist 위치 복사
      placehorderEl.style.top = itemlist.style.top;

      itemlist.before(placehorderEl);    //선택한 요소 앞에 컨텐츠 삽입


      function dragHandler(event) {
        itemlist.style.left = `${event.clientX - shiftX}px`
        itemlist.style.top = `${event.clientY - shiftY}px`

        //주어진 창 기준 좌표에서 가장 많이 중첩된 요소를 반환
        ...
        //부모를 따라서 가장 가까운 해당 요소 찾기
        let dropAreaEl = elemBelow.closest(".placehorder") //가이까 가면 dropArea 반환

        if (itemlist === dropAreaEl) {
          return
        }

        console.log("itemList 목록 찾았다!", dropAreaEl)

      }


      const dropHandler = () => {
        itemlist.style.zIndex = orgZIndex;
        document.removeEventListener("mouseup", dropHandler);
        document.removeEventListener("mousemove", dragHandler);
      };
